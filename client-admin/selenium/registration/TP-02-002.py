...
@pytest.mark.parametrize(
    ...
    [
        "Tester#Su",
        "",
        "Tester Suuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuu",
    ],
)

def test_blank_full_name_rejected(driver, invalid_name):
    ...
    name.clear(); name.send_keys(invalid_name)
    ...
    # Short wait for any native/custom tooltip on the name field
    try:
        wait_short = WebDriverWait(driver, 2)
        wait_short.until(lambda d: name.get_attribute('validationMessage') and name.get_attribute('validationMessage') != '')
        return
    except TimeoutException:
        # No tooltip quickly — check if submission succeeded (which would be a failure)
        try:
            wait_success = WebDriverWait(driver, 5)
            wait_success.until(EC.presence_of_element_located((By.XPATH, "//*[contains(text(), 'Registration successful!') or contains(text(), 'Login') ]")))
            pytest.fail('Form succeeded unexpectedly for invalid/blank name')
        except TimeoutException:
            # No tooltip and no success -> treat as pass (stayed on signup)
